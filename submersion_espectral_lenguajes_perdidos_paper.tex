...
\begin{table}[h]
\centering
\caption{Rango efectivo de embeddings estructurales ($d=10$) para tres corpora y sus controles. La dirección invertida ($r_{\text{eff}}^{\text{real}} > r_{\text{eff}}^{\text{perm}}$) confirma detección de estructura.}
...
Rongorongo real & 3.58 & 3.34 & 1.95 \\
\midrule
RR collapsed & 3.79 & 3.65 & 2.49 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Resultado clave (Tabla \ref{tab:structural_sanity}).} El sanity check invertido \textbf{pasa para los tres corpora}: PCFG ($1.79 > 1.46 > 1.37$), Indo ($3.96 > 3.72 > 2.43$), y Rongorongo ($3.58 > 3.34 > 1.95$). La versión ``colapsada'' de Rongorongo ---donde repeticiones consecutivas se codifican como tokens compuestos (e.g., $s\_\text{REP3}$)--- muestra una separación aún más clara ($3.79 > 3.65 > 2.49$).
...
Extendemos la comparación multi-idioma de la Sección \ref{sec:multi_language_consensus} incorporando 7 lenguas europeas de control (inglés, español, alemán, ruso, francés, italiano, portugués), todas extraídas de OPUS-Tatoeba (50k oraciones cada una), para un total de 17 lenguas candidatas.

\textbf{Resultados GW multi-lateral.} Aplicamos Gromov--Wasserstein (Sección \ref{sec:gw}) con regularización $\varepsilon=0.5$ y 10 iteraciones, subsampling a 50 signos por lengua. La Tabla \ref{tab:gw_17candidates} muestra la distancia GW entre Rongorongo real y cada lengua candidata.
...
El corpus RR-corpus (Sección \ref{sec:rongorongo_real}) incluye transcripciones XML con paths SVG para cada glifo de las 6 tabletas disponibles. Extraemos 8 features iconográficos por glifo:
...
\textbf{Rango efectivo.} Los features iconográficos solos producen $r_{\text{eff}} = 1.31$, dominado por una sola dimensión (tamaño del glifo). La combinación estructura + iconografía ($d=18$) produce $r_{\text{eff}} = 1.33$, consistente con la ortogonalidad observada.
