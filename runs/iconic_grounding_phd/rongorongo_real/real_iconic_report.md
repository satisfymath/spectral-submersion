...
- NegCtrlGap: 143.280 (very_strong_check_leakage)
...
C2.5 is intentionally blocked in this run because real cross-script validation is below the required threshold or another Definition 13.1 criterion failed. The very high negative-control gap is also flagged as `very_strong_check_leakage`, so these rankings should be treated as candidates for inspection and validation, not as semantic claims.
