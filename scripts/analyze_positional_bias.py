        ...
        rows.append(
            {
                ...
                "start_score": (
                    s["first"] / count - 1 / len(s["positions"])
                    if s["positions"]
                    else 0
                ),
                "end_score": (
                    s["last"] / count - 1 / len(s["positions"]) if s["positions"] else 0
                ),
            }
        )
...
def plot_positional_bias(
    df: pd.DataFrame, title: str, output_path: str, top_n: int = 30
):
    ...
    ax.barh(
        y, sub["mean_relative_position"], color=colors, alpha=0.8, edgecolor="black"
    )
    ax.set_yticks(y)
    ...
    ax.text(
        0.5, -1.5, "MIDDLE", ha="center", fontsize=9, color="#A23B72", fontweight="bold"
    )
    ax.text(
        5 / 6, -1.5, "END", ha="center", fontsize=9, color="#F18F01", fontweight="bold"
    )
...
def main():
    ...
    parser.add_argument(
        "--output-table", default="reports/tables/positional_analysis.csv"
    )
    ...
    print(
        result_df.head(20)[
            ["token", "count", "mean_relative_position", "first_ratio", "last_ratio"]
