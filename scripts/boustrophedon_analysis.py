    ...
    for i in range(cooc.shape[0]):
        for j in range(cooc.shape[1]):
            if cooc[i, j] > 0 and row_sums[i] > 0 and col_sums[j] > 0:
                pmi[i, j] = np.log2(cooc[i, j] * total / (row_sums[i] * col_sums[j] + 1e-10))
...
def main():
    ...
    print(f"Rongorongo: {len(rr_seqs)} lines, "
          f"{sum(len(s) for s in rr_seqs)} total tokens")
    print(f"Line lengths: min={min(len(s) for s in rr_seqs if s)}, "
    ...
    print(f"  {'Boustrophedon IMPROVES structure' if delta < 0 else 'Boustrophedon does NOT improve structure'}")
    ...
    print(f"  {'Boustrophedon has more structured cross-line patterns' if h_boust < h_linear else 'Linear reading has more structured cross-line patterns'}")
    ...
    for i in range(len(rr_seqs) - 1):
        ...
        if i % 2 == 0:
            start_boust = seq_next[-1]  # even line ends, next (odd, reversed) starts from end
        else:
            start_boust = seq_next[0]    # odd line (reversed) ends, next (even) starts from start
    ...
    print(f"  Mean within-line entropy (linear reading): {np.mean(within_line_entropies_linear):.3f}")
    print(f"  Mean within-line entropy (boustrophedon reading): {np.mean(within_line_entropies_boust):.3f}")
    ...
    if even_entropies_fwd:
        ...
        print(f"    {'Reversal REDUCES entropy (boustrophedon plausible)' if np.mean(even_entropies_rev) < np.mean(even_entropies_fwd) else 'Reversal INCREASES entropy (boustrophedon less plausible)'}")
    ...
    for doc_id in sorted(rr_df['doc_id'].unique()):
        doc_seqs = [seq for seq, df_seq in 
                    [(s, rr_df[rr_df['doc_id'] == doc_id]) for s in rr_seqs]
                    if len(seq) > 0][:1]  # just check if doc exists
        ...
        print(f"  Tablet {doc_id}: {n_lines} lines, {total_tokens} tokens, "
