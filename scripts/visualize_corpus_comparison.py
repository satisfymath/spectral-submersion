...
def plot_corpus_comparison(output_path: str):
    ...
    fig.suptitle(
        "Corpus Comparison: Synthetic vs Rongorongo-like vs Indus Real",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for corpus_name, csv_path, color in [
        ...
        (
            "Rongorongo-like",
            "data/raw/lost_language/corpus_rongorongo_v2.csv",
            "#A23B72",
        ),
    ...
    for corpus_name, sv_path, color in [
        (
            "Synthetic PCFG",
            "data/processed/singular_values_synthetic_v2.npy",
            "#2E86AB",
        ),
        (
            "Rongorongo-like",
            "data/processed/singular_values_rongorongo_v2.npy",
            "#A23B72",
        ),
    ...
    if controls_data:
        ...
        width = 0.25
        colors = {
            "Synthetic PCFG": "#2E86AB",
            "Rongorongo-like": "#A23B72",
            "Indus Real": "#F18F01",
        }
        for i, corpus in enumerate(["Synthetic PCFG", "Rongorongo-like", "Indus Real"]):
            ...
            vals = [
                (
                    sub[sub["variant"] == v]["effective_rank"].values[0]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate corpus comparison visualizations"
    )
