...
def main():
    ...
    n_common = min(E_struct.shape[0], E_struct_perm.shape[0],
                    E_struct_unif.shape[0], E_icon.shape[0])
    ...
    print(f"  Inverted check: real({r_eff_struct:.2f}) > perm({r_eff_struct_perm:.2f}) > unif({r_eff_struct_unif:.2f}) ? "
    ...
    for label, E in [
        ...
        print(f"  {label:20s}: r_eff={r_eff:.2f}, top-2={frac2:.1f}%, top-5={frac5:.1f}%")
        print(f"  {label:20s}: r_eff={r_eff:.2f}, top-2={frac2:.1f}%, top-5={frac5:.1f}%")
    ...
    for w_struct in [0.0, 0.25, 0.5, 0.75, 1.0]:
        ...
        results.append({
            "w_struct": w_struct, "w_icon": w_icon,
            "r_real": r_real, "r_perm": r_perm, "r_unif": r_unif,
            "pass": passed,
        })
        print(f"  w_struct={w_struct:.2f}, w_icon={w_icon:.2f}: "
    ...
    feature_names_s = ["log_freq", "type_token", "first_ratio", "last_ratio",
                       "pos_mean", "pos_std", "rep_rate", "mean_run_len",
                       "succ_entropy", "freq_rank_norm"]
    feature_names_i = ["width", "height", "aspect_ratio", "path_complexity",
                       "stroke_density", "n_components", "fill_ratio", "symmetry"]
    ...
    for idx in flat_idx[:5]:
        ...
        print(f"    {feature_names_s[i_s]:15s} <-> {feature_names_i[i_i]:20s}: r={cross_corr[i_s, i_i]:.4f}")
