...
class RongorongoGlyphRenderer:
    ...
    def draw_noun(self, variant=1):
        ...
        if variant == 1:  # n01: figura humana sentada (proto-anthropomorfo)
            # Cabeza
            ax.add_patch(
                Ellipse((5, 7.5), 2, 1.5, fill=False, linewidth=2, color="black")
            )
    ...
    def draw_name(self, variant=1):
        ...
        if variant == 1:  # p01: figura humana prominente
            ...
            ax.add_patch(
                Ellipse((4, 7), 1.5, 1.2, fill=False, linewidth=2, color="black")
            )
            ax.add_patch(
                Ellipse((6, 7), 1.5, 1.2, fill=False, linewidth=2, color="black")
            )
            ...
            ax.add_patch(
                Ellipse((5, 7), 2, 1.5, fill=False, linewidth=2, color="black")
            )
    ...
    def draw_glyph(self, code: str, output_dir: str):
        ...
        if prefix == "d":
            ...
            fig, ax = self._create_base()
            ax.text(
                5, 5, code, ha="center", va="center", fontsize=14, fontweight="bold"
            )
    ...
    def render_sequence(
        self, glyph_codes: list[str], output_path: str, title: str = ""
    ):
