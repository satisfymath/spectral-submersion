...
def main():
    ...
    for name, (path, family) in ALL_CANDIDATES.items():
        ...
        Pi = gromov_wasserstein_matrix(D_lost, D_cand, reg=2.0,
                                        max_iter=8, sinkhorn_iter=30, tol=1e-3)
        gw_dist = float(np.sum(Pi * (D_lost @ Pi @ D_cand)))
        entropy = float(-np.sum(Pi * np.log(Pi + 1e-30)))
        print(f"{name},{family},{E_cand.shape[0]},{N_SUB},{gw_dist:.6f},{entropy:.4f}", flush=True)
