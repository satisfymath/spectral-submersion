...
def main():
    ...
    C = cooccurrence_matrix_from_sequences(seq_ids, vocab_size=vocab_size, window_size=3, inverse_distance=True)
    M = ppmi_matrix(C)
    ...
    acc_at_1 = np.mean([dict_soft[i, i] == np.max(dict_soft[i]) for i in range(min(50, dict_soft.shape[0]))])
    
    # Create permutation matrix for evaluation
    n_eval = min(50, vocab_size)
    D_source = np.sqrt(np.sum((E_aligned[:n_eval, :, None] - E_aligned[:n_eval, None, :]) ** 2, axis=2) + 1e-10)
    D_target = np.sqrt(np.sum((E_trans[:n_eval, :, None] - E_trans[:n_eval, None, :]) ** 2, axis=2) + 1e-10)
    ...
    if acc_at_1 > 0.8:
        print(f"  ✓ Pipeline PASSES end-to-end: known anchors recovered with {acc_at_1:.1%} accuracy")
    elif acc_at_1 > 0.3:
        print(f"  ~ Pipeline partially works: {acc_at_1:.1%} accuracy (above random chance of {1/n_eval:.1%})")
    else:
    ...
    for n_anc in [5, 10, 20, 50]:
        ...
        dict_n = soft_dictionary(E_aligned_n[:n_eval], E_trans[:n_eval], temperature=0.1)
        acc_n = np.mean([dict_n[i, i] == np.max(dict_n[i]) for i in range(min(n_eval, dict_n.shape[0]))])
