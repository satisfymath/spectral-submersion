...
def bootstrap_r_eff_cooccurrence(sequences, vocab, n_bootstrap=200, window=3, k=16, seed=42):
    ...
    for b in range(n_bootstrap):
        ...
        for seq in boot_seqs:
            ...
            for i_pos, i in enumerate(ids):
                for j_pos in range(max(0, i_pos - window), min(len(ids), i_pos + window + 1)):
                    if i_pos != j_pos:
        ...
        for tok in vocab:
            ...
            for seq in boot_seqs:
                ...
                while j < len(seq):
                    ...
                    j = k
            tok_runs = [r for r in repeat_runs if any(seq[j:j+r].count(tok) > 0 for seq in boot_seqs)]
...
def main():
    ...
    print(f"  95% CI = [{np.percentile(r_effs_struct, 2.5):.4f}, {np.percentile(r_effs_struct, 97.5):.4f}]")
    ...
    print(f"  95% CI = [{np.percentile(r_effs_cooc, 2.5):.4f}, {np.percentile(r_effs_cooc, 97.5):.4f}]")
    ...
    results = pd.DataFrame({
        "method": ["structural"] * len(r_effs_struct) + ["cooccurrence"] * len(r_effs_cooc),
