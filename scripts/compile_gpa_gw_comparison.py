...
for rank, r in enumerate(by_rel, 1):
    ...
    print(
        f" {rank:2d}  | {r['candidate']:15s} | {r['family']:15s} | {r['n_cand']:7d} | {r['rel_dist']:.4f}  | {r['geo_dist']:.4f}{marker}"
    )
...
for rank, r in enumerate(by_gw, 1):
    marker = " ***" if r["family"] == "polynesian" else ""
    print(
        f" {rank:2d}  | {r['candidate']:15s} | {r['family']:15s} | {r['n_cand']:7d} | {r['gw_dist']:.4f}  | {r['gw_entropy']:.4f}{marker}"
    )
...
poly_names = [
    n
    for n in gpa_order
    if next(r for r in merged if r["candidate"] == n)["family"] == "polynesian"
]
...
eur_names = [
    n
    for n in gpa_order
    if next(r for r in merged if r["candidate"] == n)["family"]
    in ("romance", "germanic", "slavic")
]
...
print(
    "GPA: European avg rank {:.1f} vs Polynesian avg rank {:.1f}".format(
        np.mean(gpa_eur), np.mean(gpa_poly)
    )
)
print(
    "GW:  European avg rank {:.1f} vs Polynesian avg rank {:.1f}".format(
        np.mean(gw_eur), np.mean(gw_poly)
    )
)
print(
    "This confirms: the signal is embedding quality/confound, NOT genealogical affinity."
)
