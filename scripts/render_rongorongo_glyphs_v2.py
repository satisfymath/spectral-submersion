...
def _draw_determiner(ax, variant):
    if variant == 1:
        ...
        ax.add_patch(Polygon([(5, 7.5), (2.5, 2.5), (7.5, 2.5)], fill=False, linewidth=2, color='black'))
    elif variant == 5:
        ax.plot([5, 5], [2.5, 7.5], 'k-', linewidth=2)
        ax.plot([2.5, 7.5], [5, 5], 'k-', linewidth=2)
    else:
        ax.add_patch(Polygon([(5, 8), (2, 5), (5, 2), (8, 5)], fill=False, linewidth=2, color='black'))
...
def _draw_noun(ax, variant):
    if variant == 1:
        ...
        ax.add_patch(Polygon([(2, 5), (5, 7), (8, 5), (5, 3)], fill=False, linewidth=2, color='black'))
...
def _draw_numeral(ax, variant):
    if variant == 1:
        ...
        for angle in [72*i for i in range(5)]:
            ...
            ax.plot([5, 5 + 2.5*np.cos(rad)], [5, 5 + 2.5*np.sin(rad)], 'k-', linewidth=2.5)
    else:
...
def render_glyph_catalog(output_dir: str = "reports/figures/rongorongo_glyphs_v2"):
    ...
    for glyphs, source in examples:
        render_sequence(glyphs, seq_dir / f"seq_{source.replace(' ', '_')}.png", title=f"'{source}'")
        print(f"  Generated: {source}")

    # Glifos individuales representativos
    singles = ["d01", "d03", "n01", "n06", "v01", "v07", "p01", "p14", "x02", "x05", "m01", "m03"]
