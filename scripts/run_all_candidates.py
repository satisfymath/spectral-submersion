...
def main():
    ...
    parser.add_argument("--segmented", action="store_true", help="Use segmented embeddings (_seg.npy)")
    ...
    for cand in candidates:
        ...
        cmd = [
            ...
            "--lost-embed", args.lost_embed,
            "--candidate-embed", embed_path,
            "--candidate-name", name + ("_seg" if args.segmented else ""),
            "--output", out_json,
            "--reg", "0.5",
        ]
        env = {"PYTHONPATH": "src"}
        print(f"\n>>> Comparing against {name}{suffix} ...")
        subprocess.run(cmd, cwd=Path.cwd(), env={**env, "PATH": subprocess.os.environ["PATH"]})
    ...
    summary_path = Path(args.output_dir) / f"candidate_comparison_summary{suffix_str}.csv"
