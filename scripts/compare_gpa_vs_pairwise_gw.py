...
def main():
    ...
    for name in cand_list:
        ...
        Pi = gromov_wasserstein_matrix(
            D_lost_sub, D_cand_sub, reg=2.0, max_iter=10, sinkhorn_iter=50, tol=1e-3
        )
    ...
    for rank, (_, r) in enumerate(gpa_df.iterrows(), 1):
        ...
        print(
            f"  {rank:2d}. {r['candidate']:15s} ({r['family']:15s}): rel_dist={r['rel_dist']:.4f}{m}"
        )
    ...
    for rank, (_, r) in enumerate(gw_df.iterrows(), 1):
        m = " *" if r["family"] == "polynesian" else ""
        print(
            f"  {rank:2d}. {r['candidate']:15s} ({r['family']:15s}): gw_dist={r['gw_distance']:.4f}{m}"
        )
