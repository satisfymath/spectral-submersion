...
def fig_positional_bias():
    ...
    ax.barh(y, sub["mean_relative_position"], color=colors, alpha=0.85, edgecolor="black", linewidth=0.5)
    ax.set_yticks(y)
    ...
    ax.set_title("Sesgo Posicional de Signos del Indo (corpus Parpola CISI, top 30)",
                 fontsize=12, fontweight="bold")
    ax.set_xlim(0, 1)
    ax.axvline(1/3, color="gray", linestyle="--", alpha=0.5, linewidth=1)
    ax.axvline(2/3, color="gray", linestyle="--", alpha=0.5, linewidth=1)

    ax.text(1/6, -2.5, "INICIO", ha="center", fontsize=10, color="#2E86AB", fontweight="bold")
    ax.text(0.5, -2.5, "MEDIO", ha="center", fontsize=10, color="#A23B72", fontweight="bold")
    ax.text(5/6, -2.5, "FIN", ha="center", fontsize=10, color="#F18F01", fontweight="bold")
...
def fig_entropy_curves():
    ...
    ax.set_title("Curvas de Entropía Condicional: Corpus Indo vs. Controles",
                 fontsize=12, fontweight="bold")
...
def fig_comparison_bar():
    ...
    corpora = ["PCFG Sintético", "Rongorongo-like", "Indus Real", "Posicional Sintético"]
    ...
    ax.bar(x - width, real, width, label="Real", color="#2E86AB", alpha=0.85, edgecolor="black", linewidth=0.5)
    ax.bar(x, permuted, width, label="Permutado", color="#A23B72", alpha=0.85, edgecolor="black", linewidth=0.5)
    ax.bar(x + width, random, width, label="Aleatorio uniforme", color="#C73E1D", alpha=0.85, edgecolor="black", linewidth=0.5)

    ax.set_ylabel("Rango Efectivo", fontsize=11)
    ax.set_title("Comparación de Rango Efectivo: Corpus Reales vs. Controles",
                 fontsize=12, fontweight="bold")
    ax.set_xticks(x)
    ...
    for i, (r, rnd) in enumerate(zip(real, random)):
        ...
        ax.text(i, max(r, permuted[i], rnd) + 0.3, status, ha="center",
...
def fig_network_communities():
    ...
    ax.set_title("Comunidades de Co-ocurrencia del Indo (top 40 signos)\n"
